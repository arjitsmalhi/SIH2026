// EduRise Main Application Controller
class EduSyncApp {
  constructor() {
    ...
    this.currentTheme = localStorage.getItem('edurise_theme') || 'dark';
  ...
  async init() {
    console.log('[EduRise] Initializing offline platform...');

    // 1. Immediately apply theme and bind UI events
    ...
    try {
      if (window.eduDB) {
        ...
        console.log('[EduRise] Offline Database ready');
      }
    } catch (err) {
      console.warn('[EduRise] Offline DB init issue:', err);
    }
    
    // 3. Register Service Worker if supported
    if ('serviceWorker' in navigator) {
      navigator.serviceWorker.register('./sw.js')
        .then(() => console.log('[EduRise] Service worker active'))
        .catch((err) => console.warn('[EduRise] SW registration failed:', err));
    }
  }
  ...
  showResourceReceivedToast(resource) {
    ...
    toast.innerHTML = `
      <div style="flex: 1; min-width: 0;">
        ...
        <div style="font-size: 0.75rem; color: #888888;">${resource.chapter} • Saved for Offline Study</div>
      </div>
  ...
  applyTheme(theme) {
    ...
    localStorage.setItem('edurise_theme', theme);
    document.documentElement.setAttribute('data-theme', theme);

    const metaTheme = document.getElementById('meta-theme-color');
    if (metaTheme) {
      metaTheme.setAttribute('content', theme === 'light' ? '#eef2f6' : '#000000');
    }
  ...
  updateThemeDrawerUI() {
    const themeText = document.getElementById('drawer-current-theme-text');
    ...
    if (themeText) {
      themeText.textContent = isLight 
        ? (window.i18n ? window.i18n.t('lightMode') : 'Light Mode')
        : (window.i18n ? window.i18n.t('darkMode') : 'Dark Mode');
    }

    if (themeBadge) {
      themeBadge.textContent = isLight ? 'Light' : 'Dark';
    }
    ...
    if (themeIcon) {
      if (isLight) {
        ...
        themeIcon.innerHTML = `<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 12.79A9 9 0 1 1 11.21 3 7 7 0 0 0 21 12.79z"></path></svg>`;
      }
    }
  }
  ...
  renderRadarPeerList() {
    ...
    if (window.eduTransport.isNative) {
      ...
      statusBanner.innerHTML = '<b>Bluetooth Scanner Active:</b> Searching for nearby EduRise devices...';
